...
def test_collapse_flux_closes_the_excitation_balance() -> None:
    ...
    flux = np.asarray(result.collapse_flux(key))
    ...
    assert result.collapse_flux("q.thermal_emission") is result.collapse_flux(key)


def test_collapse_flux_requires_stored_states_and_known_keys() -> None:
    qubit = DuffingTransmon(freq=5.0, anharmonicity=-0.25, levels=3, label="q", T1=20.0)
    chip = Chip([qubit], frame="rotating")
    result = QuantumSequence(chip).simulate(
...
def test_port_flux_matches_the_raw_output_flux_for_vacuum_input_only() -> None:
    ...
    np.testing.assert_allclose(quiet.collapse_flux("line"), quiet.output("line").raw_photon_flux, atol=1e-10)
    assert quiet.collapse_flux(plane) is quiet.collapse_flux("line")
    internal = np.asarray(quiet.collapse_flux("hidden.r.internal_photon_loss"))
    ...
    residual = np.asarray(result.output("line").raw_photon_flux) - np.asarray(result.collapse_flux("line"))
    assert np.max(np.abs(residual)) > 1e-4  # |S beta|^2 and the interference term, absent for vacuum input


def test_batch_collapse_flux_stacks_per_point_traces() -> None:
    ...
    stacked = np.asarray(batch.collapse_flux(key))
