...
Dispersive regime (cross-Kerr and dressed frequencies):
...
- Gambetta et al., *Phys. Rev. A* **74**, 042318 (2006),
  "Qubit-photon interactions in a cavity" — dispersive frequency shifts.
  Coupling seeding here uses a numerical signed cross-Kerr root, requiring
  a bracket across ``seed_strength_bounds`` without assuming monotonicity.
...
def _selected_parameter_names(
    ...
    """Free parameter paths in chip order and declared parameter order."""
    names = [
        f"{device.label}.{name}"
        for device in chip.devices
        for name in device.tunable_params()
        if name in device_selection.get(device.label, ())
    ]
    names.extend(
        f"{coupling.label}.{coupling.coupling_strength_name}"
        for coupling in chip.couplings
        if coupling.coupling_strength_name in coupling_selection.get(coupling.label, ())
    )
    return names
...
def _resolve_vary(
    ...
    for key, value in vary.items():
        ...
        if label in seen_labels:
            raise ValueError(f"vary has duplicate entries resolving to component {label!r}.")
        seen_labels.add(label)

        if isinstance(value, str):
            raise ValueError(
                f"vary[{label!r}] must be a collection of parameter names (e.g. ('{value}',)), "
                f"got the bare string {value!r}."
            )
        try:
            names = tuple(str(name) for name in value)
        except TypeError as exc:
            raise ValueError(f"vary[{label!r}] must be an iterable of parameter names, got {value!r}.") from exc
        ...
        if label in devices_by_label:
            ...
            if unknown:
                raise ValueError(
                    f"vary[{label!r}] names {unknown} are not tunable parameters of "
            ...
            if unknown:
                raise ValueError(
                    f"vary[{label!r}] names {unknown} are not the declared coupling-strength "
                    f"parameter of {type(coupling).__name__} {label!r}. Available: {list(available)}."
                )
            coupling_selection[label] = names
        else:
            raise ValueError(
                f"vary key {label!r} does not match any device or coupling on the chip. "
...
def _pack_initial_params(
    ...
    *,
    ...
    coupling_specs = {
        spec.label: spec for spec in target_specs if isinstance(spec.label, str) and spec.kind == "cross_kerr"
    }
    for coupling in chip.couplings:
...
def _parameter_seed_metadata(
    ...
    *,
    ...
    for name in names:
        ...
        if spec is not None and not coupling.reduces_to_crosskerr:
...
def fit_a_dress(
    ...
    *,
    ...
    evaluator: Literal["full", "local"] = "full",
    ...
    **JAX traceability.** When the chip uses a JAX-native backend, the
    ...
    if isinstance(max_hilbert_dim, bool) or not isinstance(max_hilbert_dim, Integral) or max_hilbert_dim < 1:
        raise ValueError("max_hilbert_dim must be a positive integer.")
    ...
    if vary is None:
        ...
        device_selection, coupling_selection = _resolve_vary(chip, _coerce_mapping(vary, "vary"))
    ...
    automatic_desired_plan = vary is None
    if n_free_parameters == 0:
        raise ValueError(
            "vary selects zero free parameters; fit_a_dress has nothing to optimize. "
    ...
    if underdetermined_by_count:
        message = (
            ...
            f"residuals; the fit is underdetermined by count. Select fewer parameters with vary. "
    ...
    if identifiability["rank_deficient"]:
        ...
        if automatic_desired_plan and result.status > 0:
            raise ValueError(message)
        warnings.warn(message, UserWarning, stacklevel=2)
